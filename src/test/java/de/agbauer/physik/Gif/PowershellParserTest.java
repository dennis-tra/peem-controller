...
public class PowershellParserTest {
    ...
    public void parsesApertureValue() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "ContrastAperture 500               ");
        assertEquals("500", ap.aperture);

        ap = PowershellParser.parseLine(this.ap, "ContrastAperture 800");
        assertEquals("800", ap.aperture);

        ap.aperture = null;
        ap = PowershellParser.parseLine(this.ap, "ContrastAperture ");
        ...
        ap.aperture = null;
        ap = PowershellParser.parseLine(this.ap, "ContrastAperture 800 +3");
        assertEquals("800 +3", ap.aperture);
    }

    @Test
    public void parsesExcitationValue() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "Excitation       Hg                ");
        assertEquals("Hg", ap.excitation);

        ap.excitation = null;
        ap = PowershellParser.parseLine(this.ap, "Excitation       210p  ");
        assertEquals("210p", ap.excitation);

        ap.excitation = null;
        ap = PowershellParser.parseLine(this.ap, "Excitation       210p + 410vH  ");
        assertEquals("210p + 410vH", ap.excitation);
    }

    @Test
    public void parsesSampleNameValue() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "Sample           Si_300SiO2_MoS2_P9");
        assertEquals("Si_300SiO2_MoS2_P9", ap.sampleName);
    }

    @Test
    public void parsesExposureValue() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "Integration      120s              ");
    ...
    public void parsesDateValue() throws Exception {
        ...
        ap = PowershellParser.parseLine(this.ap, "Run on 20170206-1723");
    ...
    public void parsesImageNumber() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "Imagenr          01                ");
        assertEquals(ap.imageNumber, 1);
        ap = PowershellParser.parseLine(this.ap, "Imagenr          13                ");
        assertEquals(ap.imageNumber, 13);

        ap.imageNumber = 0;
        ap = PowershellParser.parseLine(this.ap, "Imagenr                          ");
        assertEquals(ap.imageNumber, 0);

        ap.imageNumber = 0;
        ap = PowershellParser.parseLine(this.ap, "Imagenr         asdf                 ");
        assertEquals(ap.imageNumber, 0);
    }

    @Test
    public void parsesNote() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "Note                               ");
        assertEquals(null, ap.note);

        ap = PowershellParser.parseLine(this.ap, "Note          this is a test ");
        assertEquals("this is a test", ap.note);
    }

    @Test
    public void parsesExtractorValues() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "extractor   12502,2  40300");
        assertEquals(12502.2, ap.getExtractorU(), 0.01);
        assertEquals(40300, ap.extractorI, 0.01);

        ap.setExtractorU(0.0);
        ap.extractorI = 0.0;
        ap = PowershellParser.parseLine(this.ap, "extractor   ");
        assertEquals(0.0, ap.getExtractorU(), 0.01);
        assertEquals(0.0, ap.extractorI, 0.01);

        ap.setExtractorU(0.0);
        ap.extractorI = 0.0;
        ap = PowershellParser.parseLine(this.ap, "extractor  32.4 533,3 ");
    ...
    public void parsesStigmatorValues() throws Exception {
        ap = PowershellParser.parseLine(this.ap, "stigmator -2,883 -0,003 0,06  0");
